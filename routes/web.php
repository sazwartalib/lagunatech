...
/*
|--------------------------------------------------------------------------
| Staff back-office (login + authenticated app)
|--------------------------------------------------------------------------
|
| Scoped to the admin subdomain in production (see config/app.php), so the
| root domain stays free for the marketing site and customer portal.
*/
$staffRoutes = function (): void {
...
if ($adminDomain = config('app.admin_domain')) {
    Route::domain($adminDomain)->group($staffRoutes);

    // Friendly redirect for anyone hitting the old staff login on the root domain.
    Route::get('/login', fn () => redirect()->away('https://'.$adminDomain.'/login'));
} else {
    $staffRoutes();
}
