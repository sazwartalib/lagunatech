<x-layouts.guest title="Reset password">
    ...
    <form
        method="POST"
        action="{{ route('portal.password.email') }}"
    ...
    >
        ...
        <x-ui.field label="Email" name="email" required>
            <x-ui.input type="email" name="email" :value="old('email')" required autofocus placeholder="you@example.com" />
        </x-ui.field>
        ...
        <p class="text-center text-sm text-slate-500">
            <a href="{{ route('portal.login') }}" class="font-medium text-brand-600 hover:text-brand-700">Back to sign in</a>
        </p>
    </form>
</x-layouts.guest>
