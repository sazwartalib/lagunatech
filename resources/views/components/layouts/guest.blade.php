    ...
    <div class="flex min-h-full">
        ...
        <div class="relative hidden w-1/2 flex-col justify-between bg-brand-700 p-12 text-white lg:flex">
            <div class="flex items-center gap-2 text-lg font-semibold">
                <span class="grid size-9 place-items-center rounded-lg bg-white/15 text-xl">◧</span>
                {{ config('app.name') }}
            </div>
            ...
            <p class="text-sm text-brand-200">© {{ date('Y') }} {{ config('app.name') }} Sdn Bhd</p>
        </div>
        ...
        <div class="flex w-full flex-col justify-center px-6 py-12 sm:px-12 lg:w-1/2">
            <div class="mx-auto w-full max-w-sm">
                <div class="mb-8 flex items-center gap-2 text-lg font-semibold text-slate-900 lg:hidden">
                    <span class="grid size-9 place-items-center rounded-lg bg-brand-600 text-xl text-white">◧</span>
                    {{ config('app.name') }}
                </div>
